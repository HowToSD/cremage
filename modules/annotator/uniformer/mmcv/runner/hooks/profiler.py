...
class ProfilerHook(Hook):
    ...
    Args:
        ...
        activities (list[str]): List of activity groups (CPU, CUDA) to use in
            profiling. Default: ['cpu', os.environ.get("GPU_DEVICE", "cpu")].
    ...
    def __init__(self,
                 by_epoch: bool = True,
                 profile_iters: int = 1,
                 activities: List[str] = ['cpu', os.environ.get("GPU_DEVICE", "cpu")],
        ...
        for activity in activities:
            ...
            elif activity == os.environ.get("GPU_DEVICE", "cpu"):
                self.activities.append(profiler.ProfilerActivity.CUDA)
            else:
                raise ValueError(
                    f'activity should be "cpu" or os.environ.get("GPU_DEVICE", "cpu"), but got {activity}')
