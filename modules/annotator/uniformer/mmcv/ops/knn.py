...
class KNN(Function):
    ...
    def forward(ctx,
        ...
        assert center_xyz.device == xyz.device, \
            'center_xyz and xyz should be put on the same device'

        # If using CUDA and the current active CUDA device is not the one
        # center_xyz is on, make that device active
        if center_xyz.device.type == 'cuda':
            center_xyz_device_index = center_xyz.device.index if center_xyz.device.index is not None else 0
            if torch.cuda.current_device() != center_xyz_device_index:
                torch.cuda.set_device(center_xyz_device_index)
