...
# Cremage change
# Remove conflicting CLIP PATH
tmp = sys.path.copy()
sys.path.clear()
for e in tmp:
    if e.find("cremage/modules") < 0:
        sys.path.append(e)
import clip
import numpy as np
import torch
...
RESOURCES_ROOT = os.path.realpath(os.path.join(os.path.dirname(__file__), "..", "..", "..", "scripts", "util", "detection"))
