...
def zero_init_module(module):
    ...
    Added by Cremage based zero_module in original SD 1.5 code.
    """
...
class GEGLU_with_lora(nn.Module):
    """
    Cremage note: Copied from the Cremage version of SD 1.5.
    """
    def __init__(self, dim_in, dim_out,
...
class FeedForward(nn.Module):
    """
    Cremage note: This code is the same as in the SD 1.5 code I tweaked.
    """
...
class CrossAttention(nn.Module):
    ...
    def forward(
        ...
        if len(self.lora_ranks) > 0:
            del d
        del context, x  # Cremage added (in addition to above LoRA code)
        ...
        if torch.cuda.is_available() is False:  # Cremage Mac support
            q = q.half()
            k = k.half()
            v = v.half()
...
class BasicTransformerSingleLayerBlock(nn.Module):
    ...
    def __init__(
        ...
        """
        Cremage note: This module is missing the second attention block.
        """
        super().__init__()
        assert attn_mode in self.ATTENTION_MODES
        attn_cls = self.ATTENTION_MODES[attn_mode]
...
class SpatialTransformer(nn.Module):
    ...
    def __init__(
        ...
        # Cremage note:
        # Applying a fix suggested in
        # https://github.com/Stability-AI/generative-models/issues/162
        ...
        if not use_linear:
            # Cremage note: This was the code path for SD 1.5.
            ...
            # Cremage note: This is new to SDXL
            self.proj_in = nn.Linear(in_channels, inner_dim)
            for rank in self.lora_ranks:
                self.proj_in_lora_downs.append(
                    zero_init_module(nn.Linear(in_channels, rank, bias=False)))
                self.proj_in_lora_ups.append(
                    zero_init_module(nn.Linear(rank, inner_dim, bias=False)))
                self.proj_in_lora_alphas.append(torch.nn.Parameter(torch.tensor(float(rank))))
        ...
        if not use_linear:
            ...
            for rank in self.lora_ranks:
                self.proj_out_lora_downs.append(
                    zero_init_module(nn.Linear(inner_dim, rank, bias=False)))
                self.proj_out_lora_ups.append(
                    zero_init_module(nn.Linear(rank, in_channels, bias=False)))
                self.proj_out_lora_alphas.append(torch.nn.Parameter(torch.tensor(float(rank))))
    ...
    def forward(self, x, context=None):
        ...
        # Cremage changed
        if not self.use_linear:  # If use conv
            # x = self.proj_in(x)
        ...
        x = rearrange(x, "b c h w -> b (h w) c").contiguous()
        if self.use_linear:
            # x = self.proj_in(x)  # Original code
            # Cremage changes
            x_original = self.proj_in(x)
        ...
        if self.use_linear:
            # x = self.proj_out(x)  # Original code
            x_original = self.proj_out(x)  # Cremage changes
            d_sum = 0
            for i in range(len(self.lora_ranks)):
