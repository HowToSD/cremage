    ...
    def forward(
        ...
        for layer in self:
            ...
            if isinstance(module, TimestepBlock) and not isinstance(
                ...
                if th.cuda.is_available() is False:  # Cremage for Mac
                    x = x.half()
                x = layer(x, context)
            else:
                if th.cuda.is_available() is False:  # Cremage for Mac
...
class Downsample(nn.Module):
    ...
    def __init__(
        ...
        if use_conv:
            logpy.debug(f"Building a Downsample layer with {dims} dims.")
            logpy.debug(
                f"  --> settings are: \n in-chn: {self.channels}, out-chn: {self.out_channels}, "
                f"kernel-size: 3, stride: {stride}, padding: {padding}"
            )
            if dims == 3:
                logpy.debug(f"  --> Downsampling third axis (time): {third_down}")
            self.op = conv_nd(
                dims,
                self.channels,
                self.out_channels,
...
class ResBlock(TimestepBlock):
    ...
    def __init__(
        ...
        if self.skip_t_emb:
            logpy.debug(f"Skipping timestep embedding in {self.__class__.__name__}")
    ...
    def __init__(
        ...
        if self.num_classes is not None:
            if isinstance(self.num_classes, int):
                ...
                logpy.debug("setting up linear c_adm embedding layer")
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.num_classes is not None:
            ...
            if th.cuda.is_available() is False:  # Cremage Mac support
                y = y.half()
                emb = emb.half() + self.label_emb(y).half()
            else:
