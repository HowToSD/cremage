...
def load_checkpoint(model, checkpoint_path, strict=True):
    ...
    # resize_pos_embed(state_dict, model) # Cremage: Commenting out
    resize_text_pos_embed(state_dict, model)

    # Cremage LoRA changes
    # Create a list to collect the new key-value pairs
    updates = []
    updates_del = []

    for k, v in state_dict.items():
        match = re.search(r"attn.in_proj", k)
        if match:
            new_key = k.replace("attn.", "attn.multihead_attn.")
            updates.append((new_key, v.clone()))
            updates_del.append(k)

        match = re.search(r"attn.out_proj", k)
        if match:
            new_key = k.replace("attn.", "attn.multihead_attn.")
            updates.append((new_key, v.clone()))
            updates_del.append(k)
    ...
    # Cremage
    # LoRA added model contains LoRA weights that are not in the base model
    # check point, so add zero value weights.
    # These weights are overridden later when you load LoRA model(s).
    for k, v in model.named_parameters():
        if k not in state_dict:
            state_dict[k] = v

    # Cremage: Remove visual keys
    updates_del = []
    for k, v in state_dict.items():
...
def create_model_and_transforms(
    ...
    t_end = time.perf_counter()
    logger.debug(f"open_clip factory create model took {t_end-t_start} seconds")
